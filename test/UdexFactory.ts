...
import SdexPool from '../artifacts/contracts/SdexPool.sol/SdexPool.json';
...
describe("SdexFactory", function () {
    async function deployFactoryFixture() {
        const Factory: ContractFactory = await ethers.getContractFactory("SdexFactory");
    ...
    it("pool created at expected address", async function () {
        const { factory } = await loadFixture(deployFactoryFixture);
        const bytecode: string = SdexPool.bytecode;
